...
class ClassifierSuite extends FunSuite {
  ...
  test ("DecisionTree: getPurity") {
    ...
    assert (new DecisionTreeClassifier().getPurity(featureX, yThisNode, 2.7, "gini") === (0, true))
    assert (roundTupleOne(new DecisionTreeClassifier().getPurity(featureX, yThisNode, 2.2, "gini")) === (Maths.round(-4.0/15, 9), true))
    assert (new DecisionTreeClassifier().getPurity(featureX, yThisNode, 0, "gini") === (-0.48, true))
  }
